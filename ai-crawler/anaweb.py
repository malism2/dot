...
os.environ["PYPPETEER_CHROMIUM_REVISION"] = "1566149"
...
chrome_path = "C:\\Users\\Administrator\\AppData\\Local\\Google\\Chrome\\Bin\\chrome.exe"
...
class AnaWeb:
    ...
    async def scrape_website(self, url, tags, languages):
        ...
        try:
            ...
            # 设置页面视口大小并访问具体URL
            width = 960  # 默认宽度为 1920
            height = 540  # 默认高度为 1080
            ...
            screenshot_path = screenshot.replace('.png', '.webp')
            ...
            processed_languages = {'en': {'title': title,
            ...
            if languages:
                for language in languages:
                    ...
                    processed_languages[language] = {'title': processed_title,
            ...
            return {
                ...
                'screenshot_thumbnail': thumnbail_key,
                'tags': processed_tags,
                'summary': processed_languages,
            }
        except Exception as e:
    ...
    # 将json数据写入到指定目录下的文件中
    def write_to_file(self, dir_path, name, json_data):
        try:
            # 确保目录存在
            ...
            logger.info(f"JSON数据已成功写入到文件: {file_path}")
            return True
        except Exception as e:
            logger.error(f"写入JSON文件时发生异常: {e}")
            return False
